...
class WC_Gateway_GestPay_Helper {
   ...
   private function var_export( $expression ) {
    ...
    function crypt_token( $token ) {
        ...
        $first_encrypted = openssl_encrypt($token,$method,base64_decode($key1), OPENSSL_RAW_DATA ,$iv);    
        $second_encrypted = hash_hmac('sha3-512', $first_encrypted, base64_decode($key2), TRUE);
    ...
    function decrypt_token( $token ) {
        ...
        $decrypted = openssl_decrypt($first_encrypted,$method,base64_decode($key1),OPENSSL_RAW_DATA,$iv);
        $second_encrypted_new = hash_hmac('sha3-512', $first_encrypted, base64_decode($key2), TRUE);
